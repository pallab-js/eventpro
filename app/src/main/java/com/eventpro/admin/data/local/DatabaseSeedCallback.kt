...
class DatabaseSeedCallback(private val db: () -> AppDatabase) : RoomDatabase.Callback() {
    ...
    private suspend fun seed(db: AppDatabase) {
        ...
        db.transactionDao().upsertTransaction(TransactionEntity(description = "Apex Gala — Client Deposit", amountCents = 4250000L, type = "INCOME", category = "MISC", dateMillis = t(Calendar.MARCH, 1), referenceNumber = "INV-2026-001", eventId = eventId1, clientOrVendorName = "Apex Innovations"))
        db.transactionDao().upsertTransaction(TransactionEntity(description = "Meridian Launch — Full Payment", amountCents = 4200000L, type = "INCOME", category = "MISC", dateMillis = t(Calendar.APRIL, 15), referenceNumber = "INV-2026-002", eventId = eventId2, clientOrVendorName = "Meridian Corp"))
        db.transactionDao().upsertTransaction(TransactionEntity(description = "Elite Catering — Deposit", amountCents = 150000L, type = "EXPENSE", category = "CATERING", dateMillis = t(Calendar.MARCH, 10), referenceNumber = "EXP-2026-001", eventId = eventId1, clientOrVendorName = "Elite Catering Co."))
        db.transactionDao().upsertTransaction(TransactionEntity(description = "SoundWave AV — Equipment Hire", amountCents = 320000L, type = "EXPENSE", category = "LOGISTICS", dateMillis = t(Calendar.APRIL, 2), referenceNumber = "EXP-2026-002", eventId = eventId2, clientOrVendorName = "SoundWave AV"))
        db.transactionDao().upsertTransaction(TransactionEntity(description = "Petal & Bloom — Floral Deposit", amountCents = 80000L, type = "EXPENSE", category = "MISC", dateMillis = t(Calendar.MARCH, 20), referenceNumber = "EXP-2026-003", eventId = eventId1, clientOrVendorName = "Petal & Bloom Florals"))
        db.transactionDao().upsertTransaction(TransactionEntity(description = "Venue Hire — Tech Hub", amountCents = 250000L, type = "EXPENSE", category = "VENUE", dateMillis = t(Calendar.FEBRUARY, 28), referenceNumber = "EXP-2026-004", eventId = eventId2, clientOrVendorName = "Tech Hub Conference Center"))
        db.transactionDao().upsertTransaction(TransactionEntity(description = "Marketing Materials Print", amountCents = 45000L, type = "EXPENSE", category = "MARKETING", dateMillis = t(Calendar.APRIL, 10), referenceNumber = "EXP-2026-005", eventId = null, clientOrVendorName = "PrintFast Ltd"))
        db.transactionDao().upsertTransaction(TransactionEntity(description = "Event Staff — Meridian Launch", amountCents = 180000L, type = "EXPENSE", category = "STAFF", dateMillis = t(Calendar.MAY, 18), referenceNumber = "EXP-2026-006", eventId = eventId2, clientOrVendorName = "StaffPro Agency"))
